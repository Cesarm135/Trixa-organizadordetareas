...
def abrir_configuracion():
    ...
    combo_horas = ttk.Combobox(frame_superior, values=["-"] + [f"{i} h" for i in range(1,9)],
                               font=("Catamaran", 12), justify="center")
    combo_horas.pack(pady=2)
    combo_horas.current(0)
    combo_minutos = ttk.Combobox(frame_superior, values=["-","5m","10m","15m","20m","30m","40m","50m"],
                                 font=("Catamaran", 12), justify="center")
    ...
    def guardar_config():
        global config_tiempo, config_prioridad
...
ordenar_combo = customtkinter.CTkComboBox(frameAñadir, values=["Ordenar por Completada", "Ordenar por Tiempo/Prioridad"],
                                           width=150, font=("Catamaran", 12))
ordenar_combo.set("Ordenar por Completada")
ordenar_combo.pack(side="right", padx=5)
ordenar_combo.bind("<<ComboboxSelected>>", lambda event: recargarInterfaz())
        ...
        if ordenar_combo.get() == "Ordenar por Tiempo/Prioridad":
            ...
            def sort_key(task):
                ...
                try:
                    ...
                    fecha_dt = datetime(1900, 1, 1)
                return (p_val, -fecha_dt.timestamp())
...
def mostrarTarea(texto, completada, fecha, tiempo, prioridad):
    ...
    tarea_label = customtkinter.CTkLabel(tarea_frame, text=texto, font=("Catamaran", 10), fg_color="transparent")
    ...
    if completada != "Completada":
        if prioridad and tiempo:
            ...
            if prioridad == "No urgente":
                ...
                bg_color = "gray"
            info_frame = customtkinter.CTkFrame(tarea_frame, fg_color=bg_color, corner_radius=5, height=13)
